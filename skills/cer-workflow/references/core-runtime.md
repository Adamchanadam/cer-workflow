...
C 可按「探索助手自動調度」使用 inline sub-agent 作唯讀探索、證據整理或候選
分析。探索助手不是 CER 的正式 C／E／R 角色，不得寫 workspace、代替 E 或 R、
產生正式 ready/result，也不得作為 CER Reviewer 通過證據。
...
探索助手是 C 的按需唯讀輔助，預設為 `auto-idle`；不設常駐助手、使用者模式或
新 slash command。C 只有在以下條件全部成立時才啟動少量探索助手：

1. 任務至少有兩條互不依賴的唯讀探索線。
2. 每條線的目標、輸入版本、來源身份、驗收及停止條件已凍結。
3. C 同期仍有不重複的關鍵分析、守門或裁決工作。
4. 各候選可按來源獨立驗證，不依賴共享寫入、執行次序或彼此結果。
5. 預期節省時間明顯高於派工、讀回、去重及裁決成本。

任一條不成立或無法判定即保持閒置。C 可在一次有界讀取內完成並驗收的簡單任務
必須使用零名探索助手。

- C 在派工前記錄候選依賴的來源身份，分開探索範圍；助手只交候選，不修改正式
  coverage、契約、模型、生成資料、文件真源或進度。
- C 保留主要分析與最終裁決，並在助手運行時推進不重複的關鍵工作。不得以助手
  數量、相同結論或先完成者投票接納候選。
- 輸入或來源漂移時，只丟棄依賴該版本的候選；不重跑未受影響的探索線。
- 建立工具或所需能力不可用、助手失敗、逾時、來源缺失或候選不可驗證時，將該
  候選標為不可用，不重複相同失敗；C 回到一般唯讀分析。除非缺失證據本身是
  真實 blocker，探索助手失敗不得阻塞 CER。
- 助手嘗試寫入、冒充正式角色或越出凍結範圍時，C 停止該助手並丟棄候選。
