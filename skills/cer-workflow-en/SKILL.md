...
- E1 and R direct-push results. C must not automatically use `wait_threads` or `read_thread` after dispatch as the receiving mechanism. Only when a declared direct-push state transition exists, the unique thread and current cursor are known, and the platform will not automatically wake an idle C may C arm one bounded event wait for wakeup on that transition; `BATCH_RECEIVED` and the final result are separate transitions. A wait snapshot, completion status, or commentary is not ready/result evidence. After timeout, the same transition gets one recovery wait only after reconciliation and the single controlled resend with the same `messageId`; it then blocks rather than polling or monitoring.
