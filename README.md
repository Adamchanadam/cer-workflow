...
不確定就先用 `/CER-auto`；下面是人工選擇時的簡單判斷。實際路線仍由 Skill 按任務後果、權威和驗收條件裁決。
...
跑長任務時，CER 不只是把工作派出去再收結果。Controller 會守住一開始的可驗收成果：新測試、新資料或 Reviewer 發現如果會改變方向，會先分清楚是原本驗收缺陷、必要阻礙，還是只是相鄰改善。只有會影響原成果安全接納的事才進主線；其他改善會另列，不會自動變成下一批工作。這樣做是為了避免長任務越做越闊、終點一直移動，或把 token 花在不改變交付結果的支線上。

完整 Skill 內已有 12 類同類防錯機制；README 只列最容易感受到的 5 個：

- 固定成果錨：先守住你真正要的終點。
- 新發現先分類：原驗收缺陷、必要阻礙、相鄰改善分開。
- 相鄰改善不自動變主線：避免越做越闊。
- 同根因合併修：避免審閱意見一句一句追修補。
- 成果進度不等於活動完成：候選、審閱、格式通過都不自動算完成。
