...
本版把複雜任務前的層級對焦映射進 CER 執行面。重點是先看清最終成果、本輪實際產出、工作層級、真源、範圍外項、成功證據、技術 PASS 但不算進步的反例，以及停止條件，避免流程通過但產品主成果沒有實際前進。

- 複雜 ordinary、Goal 和 CER 工作法任務不得只因 route label 跳過必要層級對焦；簡單、單步、低風險且終點唯一的任務仍可內部通過，不強制顯示對焦卡
- 可見對焦卡、preflight、活的任務簡報或路線圖更新不算成果進度、驗收證據或產品品質證明
- CER 只作本地映射；不新增 slash command、role、schema 或 enum，也不改寫 `/CER-auto` 四路線
- 中英文 Skill packages 均為 VERSION `0.3.19`，各 8 files，512 mutation cases 通過
- 發布後使用者手動 UAT 仍需另行回報

## v0.3.17

本版把長任務收尾和公開 Skill runtime 語言邊界正式同步到公開版本。重點是讓 Controller 在每輪交接或收尾時更清楚地守住原本可驗收成果，而不是把每個相鄰新發現都自動變成下一批主線工作。

- 結果收尾新增精簡收尾包：集中讀回身份、交付狀態、原終點、新發現分類、可接受條件、相鄰改善和下一步用途
- 新發現要先分清原驗收缺陷、必要阻礙、相鄰改善；只有影響原成果安全接納的項目才進主線
- ordinary execution 和 Goal 不因這個 close bundle 而變成 CER；`hostId` 也不變成硬性必填或可猜測欄位
- Skill runtime 維護邊界改為英文 canonical source，同時保留中文指令、中文自然語言觸發和中文回覆
- 中英文 Skill packages 均為 VERSION `0.3.17`，各 8 files，486 mutation cases 通過
- 發布後使用者手動 UAT 仍需另行回報

## v0.3.16

本版補強公開 README 的長任務防偏說明，讓用戶更清楚 CER 不是只派工和回讀，而是會守住原本可驗收成果，先分類新發現，再決定是否進主線。

- README 新增「長任務防偏：守住終點，不追支線」段落，說明原驗收缺陷、必要阻礙、相鄰改善的分流
- 只展示 5 個最貼近用戶感受的防錯機制：固定成果錨、新發現分類、相鄰改善不自動變主線、同根因合併修、成果進度不等於活動完成
- 本版不改 CER runtime、UAT 規則、Reviewer 規則或 `/CER-auto` 路由邏輯
- 中英文 Skill packages 均為 VERSION `0.3.16`，各 8 files，432 mutation cases 通過
- 發布後使用者手動 UAT 仍需另行回報
